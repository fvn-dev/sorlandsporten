    ...
    function docController(docService) {
          ...
          return docService.getAllDocs(dm.curQ).then(function(data){
              ...
              if (data.data.documents.length){ 
                ...
                angular.forEach(dm.documents, function(doc) {
                    ...
                    switch (doc.uni) {
                      case "SSHF":
                        doc.reqMail = 'postmottak@sshf.no';
                        break;
                      default:
      ...
      function getCurrentPage(){
        ...
        return docService.getCurPg(dm.curDoc).then(function(data){
          ...
          var rxCase = new RegExp('[\\d]{2}/[\\d]{1,6}-[\\d]{1,3}', 'g');
          var caseArray = dm.curDoc.curText.match(rxCase);

          if (caseArray !== null){

            var rxTitle = new RegExp('(?!<=Dok: )([^:])+(?=Sak:)', 'g');
            var titleArray = []; 
            ...
            if (titleArray !== null){
              ...
              /*
                // two formats for reports, older format includes "Dok:" as possible stop
                // TODO: determine via doc title = year of publish...
                var rxTitleOld = new RegExp('(?!<=Sak: )([^:])+(?=Dok:)', 'g');
                var titleArrayOld = []; 
                titleArrayOld = dm.curDoc.curText.match(rxTitleOld);
                if (titleArrayOld !== null){
                  for (var i = 0; i < caseArray.length; i++) { dm.curDoc.cases.push({id: caseArray[i], title: titleArrayOld[i]  }); }
                } else {
              */
